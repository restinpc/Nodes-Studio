...
function print_product_filter($id, $url){
    ...
    <select class="input w210" name="category" id="category" onChange=\'
    ...
    $res = engine::mysql($query);
    while($data = mysql_fetch_array($res)){
        $filter .= '<select class="input w210" name="'.$data["id"].'" onChange=\'document.getElementById("filer_form_'.$id.'").submit();\'>
    ...
    if(!empty($_SESSION["details"])){
        $filter .= '
        <input type="button" class="btn w210" value="'.lang("Reset Filter").'" onClick=\'
