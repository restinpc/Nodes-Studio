        ...
        $table = '<table><tr>';
            ...
            foreach($images as $img){
                ...
                if(!empty($img)){
                    ...
                    if($i>1){
                        $table .= '<input class="btn small del_button hidden" type="button" value="'.lang("Delete").'" onClick=\'delete_image("'.$data["id"].'", "'.$i.'");\' />';
                    }
                    $table .= '</td>';
                }
            }
    ...
    <div class="product_right">
        ...
        <div id="details_button">';
            if($data["user_id"]!=$_SESSION["user"]["id"]){
                if($data["status"]=="1"){
                    $fout .= '<button class="btn w280" onClick=\'buy_now("'.$data["id"].'", "'.lang("Checkout order?").'");\'>'.lang("Buy Now!").'</button><br/><br/>';
                }
            }
        ...
        <div id="edit_details">
            <form method="POST" id="edit_details_form">
                '.lang("Title").':<br/>
                <input class="input w280" type="text" id="product_title" name="title" value="'.$data["title"].'" /><br/><br/>
                '.lang("Description").': <br/>
                <textarea class="input w280" id="product_text" name="text" >'.$data["text"].'</textarea><br/><br/>
                '.lang("Ask price").': <br/>
                <input class="input w280" type="text" id="product_price" name="price" value="'.$data["price"].'" /><br/><br/>';
            ...
            while($fdata=  mysql_fetch_array($res)){
                ...
                $select = $fdata["value"].':<br/>'
                        . '<select class="input w280" name="property_'.$fdata["id"].'">'
                        . '<option></option>';
            ...
            $fout .= '
                ...
                <button class="btn w280" onClick=\'edit_product();\'>'.lang("Save changes").'</button>
                <br/>
            </form>
        </div>
        <div class="left">
            ...
            ';
            if($data["user_id"]==$_SESSION["user"]["id"]){
                $fout .= '<br/><button id="edit_button" class="btn w280" onClick=\'edit_product("'.lang("Save Changes").'");\'>'.lang("Edit product").'</button><br/><br/>';
                if($data["status"]=="1"){
                    $fout .= '<button class="btn w280" onClick=\'deactivate("'.$data["id"].'");\' >'.lang("Click to Deactivate").'</button><br/><br/>';     
                }else{
                    $fout .= '<button class="btn w280" onClick=\'activate("'.$data["id"].'");\'>'.lang("Click to Activate").'</button><br/><br/>';
                }
            }
            $fout .= '
            </div>
...
<div class="left">';
    ...
    if($count) $fout .= '<h6 class="tal pl10">'.lang("You might also be interested in").':</h6><br/>'.$products;
    $fout .= '
